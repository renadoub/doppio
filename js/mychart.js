...
d3.csv("/data/chartDoppio.csv", type, function(error, data) {
  if (error) throw error;

  var indices = data.columns.slice(1).map(function(id) {
    return {
      id: id,
      values: data.map(function(d) {
        return {date: d.date, performance: d[id]};
      })
    };
  });
  ...
  fund.append("text")
      ...
      .attr("transform", function(d) { return "translate(" + x(d.value.date) + "," + y(d.value.performance) + ")"; })
      .attr("x", 3)
      .attr("dy", "0.35em")
